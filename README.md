...
2. Selecione um canal para enviar a embed de autorização
3. Peça aos membros que cliquem no botão "Autorizar"
...
5. Em "OAuth2" → "URL Generator", configure:
   - Scopes: `bot`, `guilds.join`, `identify`
   - Permissões: Administrator (ou permissões específicas)
6. Configure o Redirect URI: `https//seu-dominio.com/oauth2/callback` (ou seu domínio)
...
3. O backup será criado e armazenado no MongoDB
...
4. Seu token será armazenado com segurança

### Backups Automáticos

1. Execute `/backup autobackup`
2. Selecione o intervalo (1 semana, 2 semanas, 3 semanas, 1 mês)
3. Selecione os componentes
4. Os backups serão criados automaticamente
